...
# for x in np.arange(30, 100, 10):
for x in np.arange(100, 200, 10):
    read_number = x
    read_size = 1000
    window_size = read_size
    print(f'===Running with {read_number} reads of {read_size}bp===')
    ...
    full_data.loc[full_data['gene'].isin(tb_drug_resistance_genes.keys()), 'weight'] = 1.5
    # priorities = []
    ...
    covered_positions = {}
    covered_ranges = []
    for run in range(0,read_number):
        while graph_output:
            trace = go.Scatter(
...
for x in tb_drug_resistance_genes.keys():
    ...
    print('------------------')
# %%
===Running with 100 reads of 1000bp===
Output_file:100-1000
rpoB 0.99
katG 1.0
pncA 1.0
embB 1.0
