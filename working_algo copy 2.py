...
#%%
#Running with given read size and read number
read_number = 30
read_size = 1000
...
# Comment out the following line to prevent the heavier weighting on classic drug resistance genes
full_data.loc[full_data['gene'].isin(tb_drug_resistance_genes.keys()), 'weight'] = 99
full_data.loc[full_data['gene'].isin(['eis', 'ahpC', 'fapG1']), 'weight'] = 9900000

#%%
## full_data_no_duplicates = full_data.drop_duplicates(subset=['genome_pos', 'sample_id'], keep='first')
# full_data_no_duplicates = pd.DataFrame(columns=full_data.columns)
# for x in full_data['genome_pos'].unique():
#     weight = full_data[full_data['genome_pos']==x]['freq'].mean()
#     row = full_data[full_data['genome_pos']==x].head(1)
#     row['weight'] = weight
#     full_data_no_duplicates = pd.concat([full_data_no_duplicates, row])
# full_data = full_data_no_duplicates
...
for run in tqdm(range(0,read_number)):
    ...
    # print(full_data[(full_data['genome_pos']>=start) & (full_data['genome_pos']<=end)])
    print(start, end)
    print(full_data[(full_data['genome_pos']>=start) & (full_data['genome_pos']<=end)]['gene'].value_counts()) # gene covered
    print(full_data[(full_data['genome_pos']>=start) & (full_data['genome_pos']<=end)]['gene']) # gene covered
    # print('run:', run)
    # print('total_weight',full_data[(full_data['genome_pos']>=start) & (full_data['genome_pos']<=end)]['weight'].sum())
    # print('no_of_gene_covered:',full_data[(full_data['genome_pos']>=start) & (full_data['genome_pos']<=end)]['gene'].value_counts())
    # print('------------------')
    if end > 4485058:
        end = 4485058-200
    ...
    covered_positions[f'Amplicon_{run+1}'] = {'Range':{'Start': start, 'End': end}, 'Markers':full_data[(full_data['genome_pos']>= start) & (full_data['genome_pos']<=end)][['genome_pos','gene','sublin','drtype','drugs','weight']].sort_values(by=['weight']).drop_duplicates(subset=['genome_pos'], keep='first').to_dict('records')}# verbose version of output
    ...
    full_data.loc[(full_data['genome_pos']>=start) & (full_data['genome_pos']<=end), 'weight'] = full_data.loc[(full_data['genome_pos']>=start) & (full_data['genome_pos']<=end), 'weight']=0 # set the weight of the covered positions to 0
    weight_window_sum = rolling_sum(full_data, 'weight', window_size, full_data['genome_pos'].tolist())
    
#! full data needs to be reloaded after each run
# output primer info into a csv file
# seq = 1
#%%
padding = 350
...
for i, x in enumerate(covered_ranges):
    ...
    # high_b += padding
    # low_b -= padding
    # print('original_after_padding:', low_b, high_b)
...
testing.test_coverage(bed_file_path, full_data)
...
# %%
#debug
# this is to check the range of position of the classic drug resistance genes
for x in tb_drug_resistance_genes.keys():
    print(x)
    # print(full_data[full_data['gene']==x]['genome_pos'].max())
    # print(full_data[full_data['gene']==x]['genome_pos'].min())
    max_ = full_data[full_data['gene']==x]['genome_pos'].max()
    min_ = full_data[full_data['gene']==x]['genome_pos'].min()
    print(max_, min_)
    print('gene_size:',full_data[full_data['gene']==x]['genome_pos'].max()-full_data[full_data['gene']==x]['genome_pos'].min())
    print('no_of_gene_covered:',full_data[(full_data['genome_pos']>=min_) & (full_data['genome_pos']<=max_)]['gene'].value_counts())
    
    print()
    print('------------------')
# %%
full_data.loc[(full_data['genome_pos']>=2715402) & (full_data['genome_pos']<=2716716)]
# %%

# %%
full_data.loc[(full_data['genome_pos']>=2715339) & (full_data['genome_pos']<=2716339)]['genome_pos'].unique()

# %%
full_data.loc[(full_data['genome_pos']>=4327129) & (full_data['genome_pos']<=4327629)]

# %%
# chr1	2715402	2716750
# 4327129 4327629
full_data.loc[(full_data['genome_pos']>=2277169) & (full_data['genome_pos']<=2278275)]
