...
def main(args):
    ...
    #test run: python main.py design -op ./test -a 400 -sn 1 -sg rpoB,katG -nn 2 -sc
    ...
    # Iterate over each column in the DataFrame
        ...
        gene_coverage = round(amp_snp['gene'].value_counts()/full_data_cp['gene'].value_counts()*100, 1)
...
def main_amplicon_no(args):
    ...
    # test run: python main.py amplicon_no -a 400 -sc -op ./test
    full_data = pd.read_csv(args.snp_priority)
    full_data = full_data[~full_data['drugs'].isna()]
    full_data = full_data.sort_values(by=['genome_pos'])
    full_data = full_data.reset_index(drop=True)
    ...
    # graphic_output = args.graphic_option
    if args.amplicon_sequencing:
        ...
        # print(gene_coverage)
    if args.spoligotype_sequencing:
        spacers = pd.read_csv(args.spoligo_sequencing_file, sep='\t', header=None)
        spacers = np.array(spacers)
        spacers = spacers[:, 1:3]
        spacers = spacers.tolist()
        ...
        covered_ranges = Amplicon_no.place_amplicon_spol(spol_data, 1, read_size, graphic_output=False, ref_size = w.genome_size(ref_genome))
        print(covered_ranges)
...
def main_plotting(args):
    ...
    # test run: python main.py plotting -ap ./test/Amplicon_design_output/Primer_design-accepted_primers-35-400.csv -rp ./reference_design.csv -op ./test
...
if __name__ == "__main__":
    ...
    input.add_argument('-sp_f','--spoligo_sequencing_file', type = str, help = 'Custom spoligotype range files (default: TB spligotype space ranges)', default = 'spacers.bed')
    input.add_argument('-as','--all_snps', type = str, help = 'All SNPs in the reference genome', default = 'all_snps.csv')
    ...
    input=parser_sub.add_argument_group("input options")
    input.add_argument('-s','--snp_priority', type = str, help = 'SNP priority CSV files (default: collated global 50k clinical TB samples)', default='variants.csv')
    input.add_argument('-sc_f','--spoligo_sequencing_file', type = str, help = 'Custom spoligotype range files (default: TB spligotype space ranges)', default = 'spacers.bed')
    input.add_argument('-ref','--reference_genome', type = str, help = 'reference fasta file (default: MTB-h37rv genome)', default='MTB-h37rv_asm19595v2-eg18.fa')
    ...
    input=parser_sub.add_argument_group("input options")
    input.add_argument('-s','--snp_priority', type = str, help = 'SNP priority CSV files (default: collated global 50k clinical TB samples)', default='variants.csv')
    input.add_argument('-gff','--gff_features', type = str, help = 'genomic feature file .gff for the corresponding genome', default='MTB-h37rv_asm19595v2-eg18.gff')
