...
def main(args):
    ...
    if len(specific_gene)>0:
        ...
        covered_positions_sp, covered_ranges_sp, specific_gene_data, primer_pool, accepted_primers, no_primer_ = w.place_amplicone(specific_gene_data, specific_gene_amplicone, read_size, primer_pool, accepted_primers, no_primer_, w.genome_size(ref_genome), args.graphic_option, padding=padding, output_path =output_path)
        ...
        covered_positions_nosp, covered_ranges_nosp, full_data_cp, primer_pool, accepted_primers, no_primer_ = w.place_amplicone(non_specific_gene_data, non_specific_amplicone, read_size, primer_pool, accepted_primers, no_primer_, w.genome_size(ref_genome), args.graphic_option, padding=padding, output_path =output_path)
...
if __name__ == "__main__":
    ...
    setting.add_argument('-g','--graphic_option to visualise the running of the algorithm', action='store_true', default = False, help = 'output graphic on amplicone coverage')
    ...
    setting.add_argument('-sp','--spoligotype', action='store_true', help = 'Whether to do a separate run on chekcing the number of amplicone needed to cover the spligotypes', default= False)
